...
if [ $# -lt 1 ]; then
  echo "Usage: $0 path/to/jumbotron_raw.glb" >&2
  exit 1
fi
...
# Ensure CLI present via npx (non-interactive)
...
echo "[3/6] Simplify 0.4"
$NPX simplify "$OPT" "$LOW" --ratio 0.4

echo "[4/6] Simplify 0.25"
$NPX simplify "$LOW" "$LOW2" --ratio 0.25
